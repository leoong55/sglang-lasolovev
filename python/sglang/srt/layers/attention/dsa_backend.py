...
class DeepseekSparseAttnBackend(
    ...
    def __init__(
        ...
        # Decode gathers the CP/TP-sliced heads across the DCP group;
        # CP prefill keeps the full per-rank attention head layout.
        ...
        if self.dcp_enabled:
            if model_runner.server_args.enable_hierarchical_cache:
                raise ValueError(
                    "This DSA DCP backport does not support HiCache. "
                    "Remove --enable-hierarchical-cache."
                )
            if (
