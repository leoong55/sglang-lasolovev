    ...
    if _is_hip or is_fp8_kv:
        if is_fp8_kv:
            if q.dtype != kv.dtype:
                q = q.to(kv.dtype)
            if not _is_hip:
                # CUDA launch geometry follows the actual GPU, including H200.
                # Conservative initial tile; tune only after numerical validation.
                cu = torch.cuda.get_device_properties(q.device).multi_processor_count
                block_I, threads, block_per_cu = 64, 256, 1
