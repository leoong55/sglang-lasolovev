...
report = {
    ...
    'gpu': command(['nvidia-smi', '--query-gpu=name,uuid,memory.total,driver_version', '--format=csv,noheader']),
...
try:
    ...
    report['config'] = cfg
    if cfg.get('num_hidden_layers') != 78 or 'GlmMoeDsaForCausalLM' not in cfg.get('architectures', []):
    ...
    for name in names:
        ...
        data_end = max((v['data_offsets'][1] for k, v in header.items() if k != '__metadata__'), default=0)
        if path.stat().st_size < 8 + size + data_end:
            errors.append('Truncated shard: ' + name)
        shards.append({'name': name, 'bytes': path.stat().st_size, 'header_sha256': hashlib.sha256(header_bytes).hexdigest(), 'tensor_count': len(expected)})
    report['shards'] = shards
    report['weight_bytes'] = sum(s['bytes'] for s in shards)
    report['tokenizer_files'] = {p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in root.iterdir() if p.is_file() and (p.name.startswith('tokenizer') or p.name in ('generation_config.json', 'special_tokens_map.json', 'chat_template.jinja'))}
...
if report['gpu']['returncode'] or len(gpu_lines) != 8 or not all('H200' in line for line in gpu_lines):
