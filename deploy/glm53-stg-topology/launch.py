...
def make_launch(profile, model_path, hicache=False, port=8080, environ=None):
    ...
    if conflicts:
        raise ValueError("Inherited experiment overrides are forbidden: " + ", ".join(conflicts))
    ...
    graphs = {
        "decode": {"backend": "full", "max_bs": local_limit, "bs": list(range(1, local_limit + 1))},
        "prefill": {"backend": "disabled"},
    }
    command = [
        sys.executable, "-m", "sglang.launch_server",
        "--model-path", str(model_path), "--served-model-name", "GLM-5.3",
        "--host", "0.0.0.0", "--port", str(port),
        "--tp-size", str(p["tp"]), "--pp-size", str(p["pp"]),
        "--dp-size", str(p["dp"]), "--ep-size", str(p["ep"]),
        "--attn-cp-size", "1", "--dcp-size", "1", "--moe-dp-size", "1",
        "--moe-a2a-backend", "none",
        "--mem-fraction-static", "0.80", "--max-running-requests", "48",
        "--chunked-prefill-size", "16384", "--context-length", "98304",
        "--page-size", "64", "--kv-cache-dtype", "fp8_e4m3",
        "--quantization", "w4afp8", "--disable-shared-experts-fusion", "--trust-remote-code",
        "--attention-backend", "dsa", "--dsa-prefill-backend", "flashmla_kv",
        "--dsa-decode-backend", "flashmla_kv",
        "--reasoning-parser", "glm45", "--tool-call-parser", "glm47",
        "--cuda-graph-config", json.dumps(graphs, separators=(",", ":")),
        "--random-seed", "0", "--enable-metrics", "--enable-cache-report",
    ...
    env["PYTHONPATH"] = bundle + (os.pathsep + env["PYTHONPATH"] if env.get("PYTHONPATH") else "")
...
def validate_revision(bundle, environ=None, required=True):
    ...
    if required and not expected:
        raise ValueError("SOURCE_COMMIT must declare the expected image source revision")
    if expected is not None and expected != actual:
        raise ValueError(f"Image source revision mismatch: baked={actual}, expected={expected}")
    return actual
...
def main():
    ...
    actual_revision = validate_revision(Path(__file__).resolve().parent, required=not args.print_command)
    record = {
        "base_commit": BASE_COMMIT, "profile": args.profile, "hicache": args.hicache,
        "image_source_commit": actual_revision, "expected_source_commit": os.environ.get("SOURCE_COMMIT"),
        "command": command, "pp_layer_partition": env.get("SGLANG_PP_LAYER_PARTITION"),
        "observer": env["GLM53_PP_OBSERVER"] == "1",
        "expected_local_running_limit": 24 if args.profile == "pp2" else 48 // PROFILES[args.profile]["dp"],
