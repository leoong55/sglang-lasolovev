...
def main():
    ...
    p.add_argument('--part', type=int, default=-1, help='-1 prints the manifest; otherwise a zero-based part')
    args = p.parse_args()
    ...
    with tempfile.TemporaryFile() as tmp:
        ...
        manifest = {'run_id': args.run_id, 'part_bytes': chunk_size, 'parts': parts, 'sha256': digest.hexdigest(), 'bytes': tmp.tell()}
        if args.part < 0:
            ...
            print('GLM53_EXPORT_PART ' + json.dumps({'index': args.part, 'sha256': parts[args.part], 'data': base64.b64encode(raw).decode()}), flush=True)
