...
class RenderContract(unittest.TestCase):
    def test_serving_owns_only_dedicated_resources(self):
        objects = render.serving(
            "pp2",
            "ghcr.io/leoong55/sglang-lasolovev@sha256:" + "a" * 64,
            "b" * 40,
            "node1",
        )
        for obj in objects:
            ...
            self.assertEqual(
                obj["metadata"]["labels"]["app.kubernetes.io/part-of"], render.PART
            )
        ...
        self.assertTrue(
            next(m for m in c["volumeMounts"] if m["name"] == "model")["readOnly"]
        )
    ...
    def test_unpinned_image_rejected(self):
        with self.assertRaises(ValueError):
            render.serving(
                "dpa8", "ghcr.io/leoong55/sglang-lasolovev:latest", "b" * 40, "node1"
            )
