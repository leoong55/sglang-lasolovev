...
class ExportTests(unittest.TestCase):
    def test_parts_reconstruct_stable_checksummed_archive(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            argv = [sys.executable, str(ROOT / 'export.py'), '--root', directory, '--run-id', 'run1']
            manifest = json.loads(subprocess.check_output(argv, text=True).split(' ', 1)[1])
            parts = []
            for i, expected in enumerate(manifest['parts']):
                row = json.loads(subprocess.check_output([*argv, '--part', str(i)], text=True).split(' ', 1)[1])
    ...
    def test_parent_path_refused(self):
        with tempfile.TemporaryDirectory() as directory:
            p = subprocess.run([sys.executable, str(ROOT / 'export.py'), '--root', directory, '--run-id', '..'], capture_output=True)
