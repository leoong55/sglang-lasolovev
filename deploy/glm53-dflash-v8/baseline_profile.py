...
def validate(argv):
    ...
    for token in remaining:
        ...
        if (
            option.startswith("--hicache")
            or option in ("--enable-hierarchical-cache", "--enable-hisparse")
        ):
            p.error("DFlash stage requires HiCache and HiSparse disabled")
    ...
    print(
        f"glm53-cp8-dcp4-q8-v3: full GLM-5.3; TP8 EP8 CP8 DCP4 DP1; FP8 KV; prefill={args.dsa_prefill_backend}; decode=flashmla_kv; DFlash2; no HiCache",
        flush=True,
    )
