...
def packed_rows(n, salt=0):
    return (
        (torch.arange(n * WIDTH).reshape(n, 1, WIDTH) * 37 + salt)
        .remainder(256)
        .to(torch.uint8)
    )
...
class TestPreparedDCPGather(unittest.TestCase):
        ...
        for size in (2, 4, 8):
            ...
            for rank in range(size):
                with self.subTest(size=size, rank=rank):
                    ...
                    indices = (
                        torch.randperm(
                            total // size, generator=torch.Generator().manual_seed(rank)
                        ).to(torch.int32)
                        + 5
                    )
                    ...
                    for layer in (1, 2):
                        ...
                        def gather(out, local):
                            torch.testing.assert_close(
                                local, expected[rank::size], rtol=0, atol=0
                            )
                            out.copy_(
                                torch.cat([expected[r::size] for r in range(size)])
                            )

                        parallel.dcp_group.all_gather_into_tensor = MagicMock(
                            side_effect=gather
                        )
                        actual = torch.full_like(expected, 255)
                        with patch.object(
                            dcp_comm, "get_parallel", return_value=parallel
                        ):
                            dcp_comm._gather_packed_prefix_with_plan(
                                persistent, indices, actual, plan
                            )
                            # Compare to the previous generic implementation too.
                            generic = dcp_comm.all_gather_kv_cache_for_dcp(
                                persistent[indices.long()],
                                None,
                        ...
                        ptrs = tuple(
                            t.data_ptr() for t in next(iter(plan.workspaces.values()))
                        )
    ...
    def test_packed_extend_uses_plan_updates_layer_and_zeroes_tail(self):
        ...
        plan = dcp_metadata.DCPPrefillGatherPlan(total, size, rank)
        parallel = SimpleNamespace(
            dcp_size=size, dcp_rank=rank, dcp_group=SimpleNamespace()
        )
        ...
        for layer in (2, 7):
            ...
            pool = SimpleNamespace(
                ...
                get_key_buffer=MagicMock(
                    return_value=expected[rank::size]
                    .contiguous()
                    .view(torch.float8_e4m3fn)
                ),
            )
            parallel.dcp_group.all_gather_into_tensor = lambda out, inp: out.copy_(
                torch.cat([expected[r::size] for r in range(size)])
            )
            ...
            with (
                ...
                dcp_comm.all_gather_kv_cache_for_mla_extend(
                    ...
                    [0, 256],
                    torch.arange(total // size, dtype=torch.int32),
                    total,
                    buffer,
                    512,
                    torch.empty(current, 1, 512),
                    torch.empty(current, 1, 64),
                    plan,
                )
            ...
            torch.testing.assert_close(
                actual[total : total + current],
                torch.cat((nope, rope), -1),
                rtol=0,
                atol=0,
            )
    ...
    def test_zero_prefix_avoids_collective_and_bad_plan_fails_before_collective(self):
        ...
        with patch.object(dcp_comm, "get_parallel", return_value=parallel):
            ...
            dcp_comm._gather_packed_prefix_with_plan(
                packed_rows(4), torch.empty(0, dtype=torch.int32), packed_rows(0), plan
            )
            ...
            with self.assertRaisesRegex(RuntimeError, "does not match"):
                dcp_comm._gather_packed_prefix_with_plan(
                    ...
                    torch.tensor([0, 1]),
                    packed_rows(4),
                    dcp_metadata.DCPPrefillGatherPlan(4, 4, 0),
                )
    ...
    def test_workspaces_separate_by_stream_and_batch(self):
        ...
        group = SimpleNamespace(
            all_gather_into_tensor=lambda out, inp: out.copy_(
                torch.cat([expected[r::4] for r in range(4)])
            )
        )
        parallel = SimpleNamespace(dcp_size=4, dcp_rank=0, dcp_group=group)
            ...
            for plan in plans:
                for stream in (11, 22, 11):
                    with patch.object(
                        torch.cuda,
                        "current_stream",
                        return_value=SimpleNamespace(cuda_stream=stream),
                    ):
                        out = torch.empty_like(expected)
                        dcp_comm._gather_packed_prefix_with_plan(
                            expected[::4], torch.tensor([0, 1]), out, plan
                        )
                        torch.testing.assert_close(out, expected, rtol=0, atol=0)
                self.assertEqual(len(plan.workspaces), 2)
        all_ptrs = [
            t.data_ptr() for p in plans for pair in p.workspaces.values() for t in pair
        ]
...
class TestPreparedCPGather(unittest.TestCase):
        ...
        for rank in range(size):
            with self.subTest(total=total, physical=physical, rank=rank, dtype=dtype):
                ...
                meta = cp_metadata.InterleaveContextParallelMetadata(
                    ...
                    per_rank_actual_token=[physical] * size,
                ...
                for layer in (1, 2):
                    ...
                    expected = (
                        torch.arange(total * shape * 2).reshape(total, shape * 2)
                        + layer
                    ).to(dtype)[:, ::2]
                    chunks = []
                    for r in range(size):
                    ...
                    with (
                        patch.object(cp_gather, "get_parallel", return_value=parallel),
                        patch.object(
                            cp_gather,
    ...
    def test_shape_stream_and_logical_length_changes_do_not_share_storage(self):
        ...
        meta = cp_metadata.InterleaveContextParallelMetadata(
            total_seq_lens=16,
            per_rank_actual_token=[2] * 8,
            per_rank_logical_token=[2] * 8,
        )
        parallel = SimpleNamespace(dcp_enabled=True, attn_cp_group=object())
        with (
            patch.object(cp_gather, "get_parallel", return_value=parallel),
            patch.object(
                cp_gather,
                "attn_cp_all_gather_into_tensor",
                side_effect=lambda out, inp: out.copy_(torch.cat([inp] * 8)),
            ),
            patch.object(torch.Tensor, "is_cuda", new=property(lambda self: True)),
        ):
            for stream, width, logical in (
                (11, 576, 2),
                (11, 128, 2),
                (22, 576, 2),
                (11, 576, 1),
                ...
                with patch.object(
                    ...
                    result = strategy._gather_interleaved_tensor(
                        torch.full((logical, width), 3),
                        SimpleNamespace(attn_cp_metadata=meta),
                    )
                    torch.testing.assert_close(
                        result, torch.full((logical * 8, width), 3)
                    )
        self.assertEqual(len(meta.gather_workspaces), 4)
        pointers = [
            t.data_ptr() for pair in meta.gather_workspaces.values() for t in pair
        ]
