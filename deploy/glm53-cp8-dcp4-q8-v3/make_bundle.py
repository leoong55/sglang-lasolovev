...
PREVIOUS = "819468af5a3b2b490a0ab546f12f12069c6c108b"
...
def main():
    ...
    if git("status", "--porcelain", "--untracked-files=normal", "--",
           "python/sglang", str(source.relative_to(repo))).strip():
        parser.error("Commit runtime and this build kit first so REVISION.json identifies the exact source")
    ...
    (output / "v2-to-v3.patch").write_bytes(git("diff", "--binary", PREVIOUS, "--", "python/sglang"))
    (output / "REVISION.json").write_text(json.dumps(dict(
        ...
        gpu_validated=False, image_built_here=False, profile="glm53-cp8-dcp4-q8-v3"), indent=2) + "\n")
