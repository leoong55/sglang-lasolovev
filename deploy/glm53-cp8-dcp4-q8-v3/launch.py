...
def validate(argv):
    ...
    for key, value in expected.items():
        if getattr(args, key) != value:
            p.error(f"v3 profile requires --{key.replace('_', '-')} {value}")
    ...
    for token in remaining:
        ...
        if (option.startswith("--speculative") or option.startswith("--hicache") or
            ...
            p.error("v3 requires speculative decoding, HiCache and HiSparse disabled")
    ...
    print(f"glm53-cp8-dcp4-q8-v3: full GLM-5.3; TP8 EP8 CP8 DCP4 DP1; FP8 KV; prefill={args.dsa_prefill_backend}; decode=flashmla_kv; no HiCache/spec", flush=True)
