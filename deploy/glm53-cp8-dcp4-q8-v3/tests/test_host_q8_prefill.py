...
NAMES = {
    "_dsa_impl_for_batch", "get_topk_transform_method", "forward_extend",
    "_forward_dcp_sparse_q8", "get_device_int32_arange",
}
source = extract("srt/layers/attention/dsa_backend.py", NAMES,
    class_name="DeepseekSparseAttnBackend", namespace={
        "ForwardMode": Mode, "TopkTransformMethod": Transform,
...
class TestDCPQ8Prefill(unittest.TestCase):
            ...
            for cp in (False, True):
                with self.subTest(mode=mode, cp=cp):
                    fb = SimpleNamespace(forward_mode=mode,
                        attn_cp_metadata=object() if cp else None)
                    expected = "flashmla_sparse_q8" if mode == Mode.EXTEND and cp else "flashmla_kv"
                    self.assertEqual(b._dsa_impl_for_batch(fb), expected)
        b.dcp_enabled = False
        self.assertEqual(b._dsa_impl_for_batch(SimpleNamespace(
            forward_mode=Mode.EXTEND, attn_cp_metadata=None)), "flashmla_sparse_q8")
    ...
    def _check_forward(self, prefix_lengths, current_lengths, selected_requests):
        ...
        logical = [[10 * r + p + 1 for p in range(pre + cur)]
                   for r, (pre, cur) in enumerate(zip(prefix_lengths, current_lengths))]
        packed_order = [x for r, pre in enumerate(prefix_lengths) for x in logical[r][:pre]]
        packed_order += [x for r, pre in enumerate(prefix_lengths) for x in logical[r][pre:]]
        position = {value: i for i, value in enumerate(packed_order)}
        kv_indices = torch.tensor([position[value] for row in logical for value in row], dtype=torch.int32)
        indptr = torch.tensor([0] + list(__import__('itertools').accumulate(map(len, logical))), dtype=torch.int32)
        dcp = SimpleNamespace(dcp_kv_indices=kv_indices, dcp_kv_indptr=indptr)
        page_table = Planner._build_dcp_prefill_page_table(Planner(),
            seq_lens=torch.tensor([len(logical[r]) for r in selected_requests]),
            dcp_meta=dcp, selected_batch_indices=selected_requests)
        ...
        gathered.view(torch.uint8)[:len(packed_order), 0, 0] = torch.tensor(packed_order, dtype=torch.uint8)
        dcp.dcp_kv_buffer = gathered
        fb = SimpleNamespace(forward_mode=Mode.EXTEND, attn_cp_metadata=object(), attn_dcp_metadata=dcp)
        b = make_backend()
        b.forward_metadata = SimpleNamespace(dcp_page_table_1=page_table,
            dsa_extend_seq_lens_list=[1] * n, cu_seqlens_q=torch.arange(n + 1))
        topk = torch.tensor([[len(logical[r])-1, 0, -1] for r in selected_requests], dtype=torch.int32)
        layer = SimpleNamespace(is_cross_attention=False, head_dim=576, v_head_dim=512,
            scaling=0.25, layer_id=7)

        def translate(**kw):
            self.assertEqual((kw['dcp_size'], kw['dcp_rank']), (1, 0))
            self.assertIs(kw['page_table'], page_table)
            return torch.stack([torch.tensor([
                int(page_table[i, j]) if j >= 0 else -1 for j in row
            ]) for i, row in enumerate(kw['topk_indices'])]).to(torch.int32)

        def consume(**kw):
            self.assertIs(kw['paged_kv_cache'], gathered)
            torch.testing.assert_close(kw['page_table_1_flattened'], torch.arange(64, dtype=torch.int32))
            ...
            for row in kw['page_table_1']:
                ...
                result.append((torch.softmax(keys * .25, dim=0) * keys).sum())
        ...
        with patch.object(source, 'transform_index_page_table_prefill', translate):
            out = b.forward_extend(q=torch.zeros((n, 2, 512)), k=None, v=None,
                q_rope=torch.zeros((n, 2, 64)), layer=layer, forward_batch=fb,
                topk_indices=topk, save_kv_cache=False)
        expected = []
        for r in selected_requests:
            keys = torch.tensor([logical[r][-1], logical[r][0]], dtype=torch.float32)
            expected.append((torch.softmax(keys * .25, dim=0) * keys).sum())
