...
dcp_metadata = extract("srt/layers/dcp/metadata.py", {
    "DCPPrefillGatherPlan", "DecodeContextParallelMetadata",
...
dcp_comm = extract("srt/layers/dcp/comm.py", {
    ...
    "all_gather_kv_cache_for_mla_extend", "_gather_packed_prefix_with_plan",
...
dcp_planner = extract("srt/layers/dcp/planner.py", {
    "prepare_decode_context_parallel_metadata",
              ...
              "create_dcp_kv_indices": None, "DecodeContextParallelMetadata": dcp_metadata.DecodeContextParallelMetadata,
              "DCPPrefillGatherPlan": dcp_metadata.DCPPrefillGatherPlan})
...
cp_metadata = extract("srt/layers/cp/interleave.py", {"InterleaveContextParallelMetadata"},
    namespace={"dataclass": dataclass, "field": field,
               "BaseContextParallelMetadata": dataclass(type("BaseCP", (), {"__annotations__": {"total_seq_lens": int, "bs": int}, "total_seq_lens": 0, "bs": 1}))})
cp_gather = extract("srt/layers/cp/interleave.py", {
    "_gather_interleaved_tensor", "_gather_interleaved_tensor_dcp",
}, class_name="InterleaveCPStrategy", namespace={"get_parallel": lambda: None,
    "is_allocation_symmetric": lambda: False,
    "use_symmetric_memory": lambda *a, **kw: nullcontext(),
    "attn_cp_all_gather_into_tensor": None})
InterleaveCPStrategy = type("InterleaveCPStrategy", (), {
    name: cp_gather.__dict__[name] for name in (
        "_gather_interleaved_tensor", "_gather_interleaved_tensor_dcp")})
