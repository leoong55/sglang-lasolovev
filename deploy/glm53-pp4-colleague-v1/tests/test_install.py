...
class PackagingTests(unittest.TestCase):
    def fixture(self, root):
        ...
        if subprocess.run(
            ["git", "cat-file", "-e", base], cwd=ROOT, capture_output=True
        ).returncode:
        ...
        for entry in manifest["files"]:
            ...
            data = subprocess.check_output(
                ["git", "show", base + ":python/sglang/" + entry["path"]], cwd=ROOT
            )
    ...
    def test_exact_install_and_idempotent_verification(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for entry in manifest["files"]:
                self.assertEqual(
                    m.digest((root / entry["path"]).read_bytes()),
    ...
    def test_bundle_overlay_matches_reviewed_source(self):
        for p in (BUNDLE / "overlay").rglob("*.py"):
            self.assertEqual(
                p.read_bytes(),
                (
                    ROOT / "python/sglang" / p.relative_to(BUNDLE / "overlay")
                ).read_bytes(),
            )
