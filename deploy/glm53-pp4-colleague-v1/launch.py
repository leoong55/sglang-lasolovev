...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=128,
        help="Decimal GB per rank; 0 disables L2",
    )
    p.add_argument("--l3-path", help="Optional mounted dedicated file-cache directory")
    p.add_argument(
        "--l3-max-size", default="250G", help="Cap per PP stage; four stages total"
    )
    p.add_argument(
        "--print-command",
        action="store_true",
        help="Print profile without starting or loading the model",
    )
    args = p.parse_args()
    ...
    if int(os.environ.get("SGLANG_PP_SHORT_BYPASS_TOKENS", "512")) not in range(
        0, 513, 64
    ):
    ...
    print(
        "PP full-need policy="
        + os.environ["SGLANG_PP_FULL_NEED"]
        + "; capacity and speed require the H200 workload test",
        flush=True,
    )
