        ...
        while i < len(lines) and not lines[i].startswith(('@@', 'diff --git')):
            ...
            if line.startswith((' ', '-')):
                if cursor >= len(source) or source[cursor] != line[1:]:
                    raise RuntimeError('Patch context mismatch; refusing a fuzzy application')
                cursor += 1
            if line.startswith((' ', '+')):
...
def install(root, bundle, verify_only=False):
    ...
    for entry in manifest['files']:
        ...
        if verify_only or actual != entry['base_sha256']:
            raise RuntimeError(f'Unsupported SGLang source: {relative}: {actual}; requires {manifest["base_commit"]}')
    ...
    for path, replacement in writes:
        ...
        try:
            ...
            if os.path.exists(temporary):
                os.unlink(temporary)
    print(f'PP4 patch verified: base={manifest["base_commit"]}, files={len(manifest["files"])}, installed={len(writes)}')
