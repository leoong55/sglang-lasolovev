...
class Profiles(unittest.TestCase):
    ...
    def test_other_scheduler_switch_is_rejected(self):
        with self.assertRaises(ValueError):
            compare.check(
                arm("pp4-decode"), arm("pp4-decode", park=True, skip=True), "skip"
            )

    def test_archive_recipe_is_not_a_clean_decode_arm(self):
        with self.assertRaises(ValueError):
            compare.check(arm("pp4-decode"), arm("pp4-archive"), "topology")

    def test_fp8_requires_real_pvc_name(self):
        with self.assertRaises(ValueError):
            render(image=IMAGE, profile="pp4-decode", weights="fp8")
    ...
    def test_local_decode_graph_covers_microbatch(self):
        for c in (40, 80, 120, 160):
            ...
            args = compare.argv_map(a["container"]["args"])
            self.assertEqual(
                args["--pp-max-micro-batch-size"], args["--cuda-graph-max-bs-decode"]
            )
    ...
    def test_no_accidental_production_selector(self):
        ...
        self.assertEqual(
            docs[0]["spec"]["selector"]["matchLabels"], docs[1]["spec"]["selector"]
        )
...
class Measurement(unittest.TestCase):
        ...
        return [
            {
                "sent": 0,
                "wanted": 300,
                "events": [[offset + n * 0.1, n] for n in range(1, 301)],
            }
    ...
    def test_common_window_uses_all_requests_and_excludes_prefill(self):
        ...
        self.assertAlmostEqual(result["decode_output_tokens_per_second"], 20, delta=0.2)
    ...
    def test_retraction_counter_is_not_a_gauge_or_creation_timestamp(self):
        self.assertEqual(
            bench.retract_counter(
                'sglang:num_retracted_reqs 9\nsglang:num_retracted_requests_total{rank="0"} 2\nsglang:num_retracted_requests_created 999'
            ),
            2,
        )
