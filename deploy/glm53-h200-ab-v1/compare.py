...
def argv_map(args):
    ...
    for value in args:
        if value.startswith("--"):
            ...
            raise ValueError("Value before argument")
        else:
...
def check(a, b, mode):
    for key in (
        ...
        if a["summary"][key] != b["summary"][key]:
            raise ValueError("Different " + key)
    if not a["summary"]["valid"] or not b["summary"]["valid"]:
        raise ValueError("Invalid benchmark arm")
    ca = a["container"]
    cb = b["container"]
    allowed_args = set()
    allowed_env = set()
    if mode == "topology":
        ...
        allowed_env = {"SGLANG_ENABLE_H200_SKIP_NOT_FITTING"}
    for field, allowed in (("args", allowed_args), ("env", allowed_env)):
        maps = [
            (
                argv_map(c[field])
                if field == "args"
                else {e["name"]: e["value"] for e in c[field]}
            )
            for c in (ca, cb)
        ]
        diff = {
            k
            for k in maps[0].keys() | maps[1].keys()
            if maps[0].get(k) != maps[1].get(k)
        }
        if diff - allowed:
            raise ValueError(
                f"Uncontrolled {field} differences: {sorted(diff-allowed)}"
            )
    for key in ("image", "resources", "volumeMounts", "command"):
        if key == "image" and mode == "image":
            continue
        if ca.get(key) != cb.get(key):
            raise ValueError("Different container " + key)
    for key in ("affinity", "nodeSelector", "volumes"):
        if a["pod"].get(key) != b["pod"].get(key):
            raise ValueError("Different pod " + key)
    ...
    if mode == "image" and ca["image"] == cb["image"]:
        raise ValueError("Image comparison has identical images")


def load(path):
    path = Path(path)
    manifest = list(yaml.safe_load_all((path / "manifest.yaml").read_text()))[0]
...
def main():
    ...
    print(
        "TTFT is reported separately and is not included in the resident decode throughput."
    )
