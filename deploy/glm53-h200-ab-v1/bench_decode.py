...
def prepare(a):
    ...
    tok = AutoTokenizer.from_pretrained(
        a.tokenizer, trust_remote_code=True, local_files_only=True
    )
    ...
    data = {
        ...
        "tokenizer_vocab_sha256": sha(
            json.dumps(sorted(tok.get_vocab().items())).encode()
        ),
        "requests": [
            rng.choices(vocab, k=a.input_tokens) for _ in range(a.concurrency)
        ],
    }
...
def summarize(rows, warmup=64, min_window=10):
    ...
    if len(complete) != len(rows):
        reasons.append("request_failure_or_missing_token_counts")
    ...
    for r in complete:
        ...
        if left is None or right is None:
            reasons.append("insufficient_warmup_or_tail")
            continue
        starts.append(left)
        ends.append(right)
    t0 = max(starts, default=0)
    t1 = min(ends, default=0)
    duration = t1 - t0
    if duration < min_window:
        reasons.append("no_sufficient_common_resident_decode_window")
    tokens = 0
    gaps = []
    per_request = []
    coalesced = False
    ...
    return {
        ...
        "window_start": t0,
        "window_end": t1,
        "window_seconds": max(0, duration),
        "decode_output_tokens": tokens,
        "decode_output_tokens_per_second": tokens / duration if duration > 0 else None,
        "per_request_decode_ms_per_token_median": (
            statistics.median(per_request) if per_request else None
        ),
        "ttft_p50_ms": percentile(
            [(r["events"][0][0] - r["sent"]) * 1000 for r in complete], 0.5
        ),
        "ttft_p95_ms": percentile(
            [(r["events"][0][0] - r["sent"]) * 1000 for r in complete], 0.95
        ),
        "stream_gap_p95_ms": percentile(gaps, 0.95),
        "stream_gap_p99_ms": percentile(gaps, 0.99),
...
def retract_counter(text):
    rows = [
        ...
        if line.startswith("sglang:num_retracted_requests_total")
    ]
    if rows:
...
async def run(a):
    ...
    if "@sha256:" not in c["image"]:
        raise ValueError("Pin the manifest image by digest before measuring")
    pods = json.loads(Path(a.pod_json).read_text())["items"]
    if len(pods) != 1:
        raise ValueError("Expected exactly one lab pod")
    pod = pods[0]
    status = next(
        x for x in pod["status"]["containerStatuses"] if x["name"] == c["name"]
    )
    actual = next(x for x in pod["spec"]["containers"] if x["name"] == c["name"])
    if (
        actual["image"] != c["image"]
        or not status.get("ready")
        or status.get("restartCount", 0) != 0
        or not status.get("imageID")
    ):
        raise ValueError(
            "Pod image/readiness/restart status does not match a clean arm"
        )
    if (
        actual.get("args") != c["args"]
        or actual.get("env") != c["env"]
        or actual.get("command") != c["command"]
    ):
    ...
    async with aiohttp.ClientSession(
        timeout=timeout, headers=headers, connector=aiohttp.TCPConnector(limit=0)
    ) as session:
        ...
        if not isinstance(server, dict):
            raise ValueError(
                "server_args is not a JSON object; inspect saved server-info"
            )
        expected = {
            "max_running_requests": len(data["requests"]),
        ...
        for key, want in expected.items():
            if key not in server or server[key] != want:
                raise ValueError(
                    f"Not a decode-only profile: effective {key}={server.get(key)!r}; expected {want!r}"
                )
        for flag in (
            "tp-size",
            "pp-size",
            ...
            want = c["args"][i + 1]
            key = flag.replace("-", "_")
            if str(server.get(key)) != str(want):
                raise ValueError(f"Effective {key} does not match manifest")
        if data["input_tokens"] + data["output_tokens"] > server.get(
            "context_length", 0
        ):
        ...
        async def request(index, ids):
            ...
            body = {
                ...
                "sampling_params": {
                    "temperature": 0,
                    "max_new_tokens": data["output_tokens"],
                    "ignore_eos": True,
                },
            }
            try:
                async with session.post(
                    a.url.rstrip("/") + "/generate", json=body
                ) as response:
                    response.raise_for_status()
                ...
                row["error"] = repr(e)
            print(
                f"completed {index+1}/{len(data['requests'])}: tokens={row['events'][-1][1] if row['events'] else 0}",
                flush=True,
            )
            return row

        rows = await asyncio.gather(
            *(request(i, ids) for i, ids in enumerate(data["requests"]))
        )
    ...
    if info.get("startup_time") != info_after.get("startup_time"):
        result["valid"] = False
        result["invalid_reasons"].append("server_restarted")
    old, new = retract_counter(before), retract_counter(after)
    result["retractions_before"] = old
    result["retractions_after"] = new
    if old is None or new is None or old != new:
        result["valid"] = False
        result["invalid_reasons"].append("retractions_or_missing_counter")
    result.update(
...
def main():
    ...
    q.add_argument("--concurrency", type=int, default=80)
    q.add_argument("--input-tokens", type=int, default=8192)
    q.add_argument("--output-tokens", type=int, default=2048)
    ...
    q.add_argument("--pod-json", required=True)
    q.add_argument("--label", required=True)
    q.add_argument("--output", required=True)
    q.add_argument("--timeout", type=float, default=3600)
    q.add_argument("--min-window", type=float, default=10)
    q.add_argument("--api-key", default=None)
    a = p.parse_args()
    if a.command == "prepare":
        if min(a.concurrency, a.input_tokens) <= 0 or a.output_tokens <= 128:
            p.error("Positive inputs and output >128 required")
        prepare(a)
    else:
        asyncio.run(run(a))
