...
def main():
    ...
    subprocess.run(['python3', str(repo/'deploy/glm53-hicache-v9/make_bundle.py'), str(out)], check=True)
    shutil.copytree(kit, out/'h200', ignore=shutil.ignore_patterns('__pycache__', '*.pyc'))
    for name in ('Dockerfile', 'build.sh'):
        shutil.copy2(kit/name, out/name)
    rev = json.loads((out/'REVISION.json').read_text())
    rev.update(profile='glm53-h200-ab-v1', fork_base_commit=BASE,
               image_built_here=False, gpu_validated=False,
               archive_patches=['5-park-chunked-prefill-adapted', '7-skip-not-fitting-corrected'],
    ...
    for row in manifest['files']:
        r = subprocess.run(['git', 'show', BASE+':'+row['path']], cwd=repo, capture_output=True)
        row['compatible_sha256'].append(hashlib.sha256(r.stdout).hexdigest() if r.returncode == 0 else None)
    (out/'base-files.json').write_text(json.dumps(manifest, indent=2)+'\n')
    sums = [f'{hashlib.sha256(f.read_bytes()).hexdigest()}  {f.relative_to(out)}'
