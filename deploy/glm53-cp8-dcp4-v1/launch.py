"""Validate the v1 launch profile, then exec SGLang as the container process."""
...
def validate(argv):
    ...
    for key, value in expected.items():
        if getattr(args, key) != value:
            p.error(f"v1 profile requires --{key.replace('_', '-')} {value}")
    for token in remaining:
        ...
        if (option.startswith("--speculative") or option.startswith("--hicache") or
            ...
            p.error("v1 requires speculative decoding, HiCache and HiSparse disabled")
    ...
    print("glm53-cp8-dcp4-v1: full GLM-5.3; TP8 EP8 CP8 DCP4 DP1; FP8 KV; flashmla_kv; no HiCache/spec", flush=True)
