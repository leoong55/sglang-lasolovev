...
dcp_metadata = extract(
    "srt/layers/dcp/metadata.py",
    {
...
dcp_planner = extract(
    ...
    namespace={
        ...
        "create_dcp_kv_indices": None,
        "DecodeContextParallelMetadata": dcp_metadata.DecodeContextParallelMetadata,
...
backend = extract(
    ...
    {
        "_build_dcp_prefill_page_table",
        "_flashmla_q_head_bucket",
        "_compute_flashmla_metadata",
    },
    class_name="DeepseekSparseAttnBackend",
    namespace={"DSAFlashMLAMetadata": SimpleNamespace},
)
DeepseekSparseAttnBackend = type(
    "DeepseekSparseAttnBackend",
    (),
    {
        name: backend.__dict__[name]
        for name in (
            "_build_dcp_prefill_page_table",
            "_flashmla_q_head_bucket",
            "_compute_flashmla_metadata",
...
cp_metadata = extract(
    "srt/layers/cp/interleave.py",
    {"InterleaveContextParallelMetadata"},
    namespace={
        "dataclass": dataclass,
        "field": field,
        "BaseContextParallelMetadata": dataclass(
            type(
                "BaseCP",
                (),
                {
                    "__annotations__": {"total_seq_lens": int, "bs": int},
                    "total_seq_lens": 0,
                    "bs": 1,
                },
            )
...
cp_gather = extract(
    "srt/layers/cp/interleave.py",
    {
        "_gather_interleaved_tensor",
        "_gather_interleaved_tensor_dcp",
    },
    class_name="InterleaveCPStrategy",
    namespace={
        "get_parallel": lambda: None,
...
InterleaveCPStrategy = type(
    "InterleaveCPStrategy",
    (),
    {
        name: cp_gather.__dict__[name]
        for name in ("_gather_interleaved_tensor", "_gather_interleaved_tensor_dcp")
    },
)
