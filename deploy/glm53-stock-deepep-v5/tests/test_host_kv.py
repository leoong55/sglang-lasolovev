...
import torch
from host_source import DeepseekSparseAttnBackend, dcp_comm, dcp_planner
...
class TestDSADCPMetadataPlanner(unittest.TestCase):
    def _prepare_metadata(
        self, kv_dtype: torch.dtype, row_width: int, prefix_lens=None
    ):
    ...
    def test_gather_plan_requires_each_prefix_aligned_not_only_the_sum(self):
        ...
        unaligned = self._prepare_metadata(
            torch.float8_e4m3fn, PACKED_KV_ROW_WIDTH, [3, 1]
        )
    ...
    def test_prefill_page_table_preserves_original_batch_segments(self):
        ...
        for batch_indices, seq_lens, expected in (
            ([0, 1, 2], [2, 3, 1], [[4, 5, -1], [10, 11, 12], [20, -1, -1]]),
...
class TestDSADCPKVBufferGather(unittest.TestCase):
    def test_dcp_gather_restores_request_order(self):
        ...
        gathered_padded = torch.tensor([0, 1, 2, 99, 10, 11], dtype=torch.float16).view(
            6, 1, 1
        )
        ...
        with (
            ...
            patch.object(
                dcp_comm, "_all_gather_dcp_kv_cache", return_value=gathered_padded
            ),
        ):
        ...
        for kv_dtype in (torch.float16, torch.bfloat16):
            with self.subTest(kv_dtype=kv_dtype):
                ...
                pool = SimpleNamespace(
                    ...
                    get_mla_kv_buffer=MagicMock(return_value=(cache_nope, cache_rope)),
                )
    ...
    def test_packed_fp8_gather_is_byte_exact(self):
        ...
        with (
            patch.object(
                dcp_comm, "all_gather_kv_cache_for_dcp", return_value=gathered_prefix
            ) as gather_mock,
            patch(
                "sglang.kernels.ops.attention.dsa.quant_k_cache.quantize_k_cache_separate",
        ...
        self.assertTrue(
            torch.equal(buffer_bytes[2:4, ..., :PACKED_NOPE_ROW_BYTES], current_nope)
        )
        self.assertTrue(
            torch.equal(buffer_bytes[2:4, ..., PACKED_NOPE_ROW_BYTES:], current_rope)
        )
