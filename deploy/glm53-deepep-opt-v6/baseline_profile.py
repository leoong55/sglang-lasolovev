...
def validate(argv):
    ...
    for key, value in expected.items():
        if getattr(args, key) != value:
            p.error(f"v6 profile requires --{key.replace('_', '-')} {value}")
    ...
    print(
        f"glm53-deepep-opt-v6: validated layout; full GLM-5.3; TP8 EP8 CP8 DCP4 DP1; FP8 KV; prefill={args.dsa_prefill_backend}; decode=flashmla_kv; no HiCache/spec",
        flush=True,
    )
