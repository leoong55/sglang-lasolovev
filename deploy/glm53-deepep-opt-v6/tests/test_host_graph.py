...
# Import only the opt-in helper; importing the sglang package requires CUDA.
opt_spec = importlib.util.spec_from_file_location(
    "sglang.srt.layers.cp.glm53_deepep", ROOT / "srt/layers/cp/glm53_deepep.py"
)
opt_helper = importlib.util.module_from_spec(opt_spec)
opt_spec.loader.exec_module(opt_helper)
with patch.dict(sys.modules, {"sglang.srt.layers.cp.glm53_deepep": opt_helper}):
...
class GraphTests(unittest.TestCase):
    ...
    def test_eager_scope_restores_capture_token_and_rejects_live_capture(self):
        ...
        func = next(
            n
            for n in tree.body
            if isinstance(n, ast.FunctionDef) and n.name == "eager_execution"
        )
        ...
        with patch.dict(
            sys.modules,
            {
                "sglang.srt.model_executor.runner_backend_utils.breakable_cuda_graph.context": SimpleNamespace(
                    suspend_breakable_cuda_graph=nullcontext
                )
            },
        ):
            ...
            with self.assertRaisesRegex(RuntimeError, "active CUDA capture"):
                with ns["eager_execution"]():
                    self.fail("entered active capture")
            self.assertIs(current.get(), original)
    ...
    def test_opt_in_support_guard_rejects_other_models_and_topologies(self):
        ...
        with (
            ...
            with patch.dict(os.environ, {"SGLANG_GLM53_DEEPEP_OPT": "1"}):
                self.assertTrue(helper.supports(None))
