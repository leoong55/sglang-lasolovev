...
def install(root, bundle, verify_only=False):
    ...
    for row in manifest["files"]:
        ...
        if verify_only or current != row["base_sha256"]:
            raise RuntimeError(f"Source mismatch: {target}; expected base {manifest['base_commit']} or this exact patch")
    ...
    try:
        ...
        raise
    print(f"glm53-cp8-dcp4-q8-v2: verified {len(manifest['files'])} files; installed {len(changed)}; package={root}", flush=True)
...
if __name__ == "__main__":
    ...
    install(args.package_root or package_root(), Path(__file__).resolve().parent, args.verify_only)
